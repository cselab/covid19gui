...
static auto make_sir_rhs(double N, double beta, double gamma,
    ...
    double tint2, double dint2, double kbeta2)
{
    /// RHS of the SIR model. We preprocess beta/N for efficiency.
    return [beta_over_N = beta / N, gamma, tint, dint, kbeta,
           tint2, dint2, kbeta2](const State &x, State &dxdt, double t)
    {
        double S = x[0];
        double I = x[1];
        double R = x[2];
        ...
        const double b_N = beta_over_N *
            trans(1., kbeta, t, tint, dint * 0.5) *
            trans(1., kbeta2 / kbeta, t, tint2, dint2 * 0.5);
        double A = b_N * S * I;
        double B = gamma * I;
        dxdt[0] = -A;
        dxdt[1] = A - B;
...
/**
  ...
  */
std::vector<double>
sir_infected_so_far(double S0, double I0, double R0, int days,
                    long long N, double beta, double gamma,
                    double tint, double dint, double kbeta,
                    double tint2, double dint2, double kbeta2)
{
    ...
    auto rhs = make_sir_rhs(N, beta, gamma,
